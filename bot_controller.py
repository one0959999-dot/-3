...
class BotController:
    ...
    def trading_job(self):
                    ...
                    if getattr(self, 'peak_total_asset', 0) > 0:
                        ...
                        if mdd <= -10.0:
                            ...
                            for core in safe_core_positions:
                                if core.shares > 0:
                                    # 🚨 변경: 하한가로 밀려서라도 무조건 즉시 체결되는 순수 시장가(01) 메서 호출
        ...
        for core in safe_core_positions:
            ...
            if not cp or cp <= 0: 
                cp = 50000  # 🚨 [강제 테스트 스위치] 장외 시간 등 가격 수신 불가 시 가상 가격 5만원 투입
        ...
        for ticker, pos in trading_sat_items:
            try:
                ...
                if not price or price <= 0: 
                    price = 50000  # 🚨 [강제 테스트 스위치] 가상 가격 투입
                ...
                if signal == 'BUY' and pos_shares == 0 and is_cooldown_passed:
                    ...
                    if self.gemini:
                        ...
                        # 🚨 [강제 테스트 스위치] AI가 "쉴래" 라며 거절해도 무조건 강제 승인 처리!
                        decision = True
                    ...
                    if self.gemini:
                        ...
                        # 🚨 [강제 테스트 스위치] AI 매도 역시 거절 무시!
                        decision = True
                        ai_reason = "테스트 목적 강제 익/손절 통과"
