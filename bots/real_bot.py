...
class RealBotController(BaseBot):
    """오직 실전투자 API 연동만을 책임지는 날렵한 자식 봇 클래스"""
    
    def __init__(self, user_id, kis_config=None, telegram_config=None, core_stocks=None):
        # 부모 클래스(BaseBot)에게 "나는 실전 모드(is_mock=False)야!"라고 알려주며 초기화합니다.
        super().__init__(user_id, kis_config, telegram_config, core_stocks, is_mock=False)
        
    def _init_api(self, kis_config):
        """부모의 빈 메서드를 채워, 실전투자 API 객체를 장착합니다."""
    ...
    def _create_websocket(self, app_key, callback):
        """부모의 빈 메서드를 채워, 실전투자 전용 웹소켓을 연결합니다."""
