...
[
  ...
  ...
        ...
        try:
            import json
            text = self.generate_content(prompt, temperature=0.2)
            if not text:
                return []
            if "```" in text:
                text = text.split("```")[1].lstrip("json").strip()
            ...
            logger.warning(f"[Gemini] ai_select_us_core_stocks 오류: {e}")
            return []

    def ai_discover_satellite_themes(self) -> list:
        if not self.client:
            return []
...
[
  ...
  ...
        ...
        try:
            import json
            text = self.generate_content(prompt, temperature=0.5)
            if not text:
                return []
            if "```" in text:
                text = text.split("```")[1].lstrip("json").strip()
            ...
            logger.warning(f"[Gemini] ai_discover_satellite_themes 오류: {e}")
        return []
