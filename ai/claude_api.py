...
[
  ...
  ...
    ...
    def ai_approve_us_trade(self, signal: str, stock_name: str, ticker: str,
        ...
        ind = indicators or {}
        mkt = market_info or {}
        decision, reason, _ = self.ai_finetune_decision(
    ...
    def ai_swing_trade_check(
        ...
        signal = 'SELL'
        decision, reason, confidence = self.ai_finetune_decision(
        ...
        import logging
        logging.getLogger('lassi_bot').info(
            f"[AI 스윙판단] {name}({ticker}) pnl={pnl_pct:+.1f}% → {'EXIT' if decision else 'HOLD'} | {reason}")

        if decision:
            # 파인튜닝이 SELL 확정 → 수익이면 SELL_REBUY 검토, 손실이면 EXIT
            return 'SELL_REBUY' if pnl_pct > 0 else 'EXIT'
        else:
            return 'ACCUMULATE' if pnl_pct < -3 and accumulate_count == 0 else 'EXIT'
