...
"""
    ...
    def ai_select_satellites(self, candidates, hot_sectors, n, sector_guide: str = ''):
        ...
        for c in candidates[:200]:
            ...
            _er = c.get('exp_return'); _ew = c.get('exp_win')
            er_tag = (f"📈예상수익:+{_er:.0f}%(승률{_ew:.0f}%,{c.get('exp_basis','')})"
                      if _er is not None else "📈예상수익:데이터부족")
            lines.append(
                f"- {c['name']}({c['ticker']}) | 섹터:{c.get('sector','-')} | "
                f"{er_tag} | 20일모멘텀:{c.get('momentum_20d',0):+.1f}% | "
  ...
  ...
    ...
    def ai_approve_us_trade(self, signal: str, stock_name: str, ticker: str,
        ...
        try:
            ...
            _sigs = ind.get('signal_types') or [signal]
            _buy_sigs = [s for s in _sigs if 'BUY' in s] or _sigs
            _fc = get_forecast('US', mkt.get('market_phase'), _buy_sigs)
        except Exception:
            pass
        _sec_stat = None
        try:
            from base.database import get_sector_phase_stat
            _sec_stat = get_sector_phase_stat('US', sector or ind.get('sector', '기타'), mkt.get('market_phase'))
        except Exception:
            pass
        decision, reason, _, _out = self.ai_finetune_decision(
            signal=signal, stock_name=stock_name, ticker=ticker,
            price=price_usd, mode='US', forecast=_fc, sector_stat=_sec_stat,
...
REASON: (1줄)"""
    ...
    def ai_finetune_decision(self, signal: str, stock_name: str, ticker: str,
                             ...
                             forecast: dict = None,
                             sector_stat: dict = None) -> tuple:
        """파인튜닝 형식 통일 판단 메서드 — KR/US 매매 승인 공통 처리.
        forecast: 백테스트 통계(get_forecast 결과) — AI가 알고리즘의 통계적 우위를 알고 판단하도록 주입(알고리즘 主)."""
        if not self.client:
        ...
        if forecast and forecast.get('n'):
            lines.append("")
            lines.append("=== 📊 백테스트 통계 (과거 동일 국면·신호 실제 결과) ===")
            _w10 = forecast.get('win10'); _w20 = forecast.get('win20')
            _tgt = forecast.get('target'); _stp = forecast.get('stop'); _hd = forecast.get('hold_days')
            lines.append(f"표본 {forecast['n']:,}건 | 10%달성률 {_w10:.0f}%" + (f" · 20%달성률 {_w20:.0f}%" if _w20 is not None else ""))
            if _tgt is not None:
                lines.append(f"평균 최대상승 +{_tgt:.0f}% / 평균 최대낙폭 {_stp:.0f}% / 평균 고점도달 ~{_hd:.0f}일")
            lines.append("→ 이 통계는 과거 동일조건의 실제 성과이며, 알고리즘이 이 통계적 우위에 근거해 신호를 냈습니다.")
            lines.append("   (알고리즘 主) 명백한 악재(공시·뉴스)나 지표 모순이 없는 한 통계적 우위를 존중하십시오.")

        if sector_stat and sector_stat.get('total_signals'):
            lines.append("")
            lines.append("=== 🏭 섹터×국면 통계 ===")
            lines.append(f"[{sector}] {market_phase_kr or market_phase} 과거 {sector_stat['total_signals']:,}건 | "
                         f"평균 최대상승 +{sector_stat.get('avg_max_gain_pct', 0):.0f}% / 최대낙폭 {sector_stat.get('avg_max_drawdown_pct', 0):.0f}%")
...
{market_data_text}"""
    ...
    def ai_approve_trade(self, signal, stock_name, ticker, price, strategy,
        ...
        try:
            ...
            _sigs = ind.get('signal_types') or [signal]
            _buy_sigs = [s for s in _sigs if 'BUY' in s] or _sigs
            _fc = get_forecast('KR', mkt.get('market_phase'), _buy_sigs)
        except Exception:
            pass
        _sec_stat = None
        try:
            from base.database import get_sector_phase_stat
            _sec_stat = get_sector_phase_stat('KR', ind.get('sector', '기타'), mkt.get('market_phase'))
        except Exception:
            pass
        decision, reason, confidence, _out = self.ai_finetune_decision(
            signal=signal, stock_name=stock_name, ticker=ticker,
            price=price, mode='KR', forecast=_fc, sector_stat=_sec_stat,
            rsi=ind.get('rsi'), macd=ind.get('macd'), macd_signal=ind.get('macd_signal'),
            bb_upper=ind.get('bb_upper'), bb_mid=ind.get('bb_mid'), bb_lower=ind.get('bb_lower'),
            sma5=ind.get('sma5'), sma20=ind.get('sma20'),
            sma60=ind.get('sma60'), sma120=ind.get('sma120'),
