...
class KisRealWebSocket:
    ...
    def _run_loop(self):
        while self.is_running:
            ...
            if not _is_market_hours():
                ...
                print(f"[WebSocket 실전] 장외 시간 — {wait//60}분 후 개장 시 재연결 예정 (불필요한 연결 차단)")
                # 1분 단위로 깨어나 is_running 체크 (stop() 호출에 즉시 반응)
            ...
            try:
                print(f"[WebSocket 실전] 증권사 통신 허브({self.url}) 연결을 수립합니다.")
                ...
                print(f"[WebSocket 실전] 네트워크 소켓 런타임 예외 발생: {e}")

            if self.is_running:
                if _is_market_hours():
                    print("[WebSocket 실전] 세션 흐름이 중단되었습니다. 5초 후 백그라운드 자동 복구를 개시합니다...")
                    time.sleep(5)
                else:
                    print("[WebSocket 실전] 장 마감 — 재연결 대기 모드로 전환합니다.")

    def _on_open(self, ws):
        print("[WebSocket 실전] 한투증권 실전투자 실시간 웹소켓 서버 연결 성공!")
        
        with self.lock:
            re_subscribe_list = list(self.subscribed_tickers)
            
        if re_subscribe_list:
            print(f"[WebSocket 실전] 시스템 재연결로 인해 기존 감시망에 있던 {len(re_subscribe_list)}개 종목을 실시간 재등록합니다.")
            time.sleep(0.5)  # 소켓 안정화 대기
            for ticker in re_subscribe_list:
                self._send_subscription_packet(ticker, tr_type="1")
                time.sleep(0.2)  # 0.08 → 0.2 (rate limit 방지)
    ...
    def _on_message(self, ws, message):
        try:
            ...
            print(f"[WebSocket 실전] 실시간 데이터 파싱 세그먼트 오류: {e}")

    def _on_error(self, ws, error):
        print(f"[WebSocket 실전 Error] 통신 에러 발생: {error}")

    def _on_close(self, ws, close_status_code, close_msg):
        print(f"[WebSocket 실전] 원격 서버가 접속 세션을 해제했습니다. (코드: {close_status_code}, 메시지: {close_msg})")
    ...
    def _send_subscription_packet(self, ticker: str, tr_type: str):
        ...
        try:
            ...
            print(f"[WebSocket 실전] 네트워크 패킷 주입 실패 ({ticker}): {e}")