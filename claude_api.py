...
- 답변은 간결하고 실용적이어야 합니다."""
    ...
    _SMART_MODEL   = os.environ.get("LASSI_CLAUDE_SMART_MODEL", "claude-sonnet-4-6")
...
■ 봇 설정 변경 (당신이 직접 가능) — 답변 마지막에 해당 블록 포함
  ...
  ② 코어 종목 교체 (사용자 요청 시 또는 AI 판단으로 교체 필요 시):
    [BOT_COMMAND]{"action":"update_core_stocks","stocks":[{"ticker":"005490","name":"POSCO홀딩스"}]}[/BOT_COMMAND]
    - stocks 배열에 ticker(종목코드)와 name(종목명) 필수
    - 한 번에 여러 종목 가능: [{"ticker":"005490","name":"POSCO홀딩스"},{"ticker":"105560","name":"KB금융"}]
...
[🚫 지어내기 엄금 — 이것이 가장 중요한 규칙]
- 존재하지 않는 버전명("플레이북 v1.3" 등), 파일명, 설정값을 절대 만들어내지 말 것
- 봇이 스캔할 종목 리스트를 임의로 나열하지 말 것 (봇은 고정 목록이 아닌 스크리너로 실시간 발굴)
- 현재 봇 상태(실행 중/정지, 매수 예정 시각 등)는 제공된 포트폴리오 컨텍스트 데이터만 참고할 것
- 확인되지 않은 수치, 일정, 예측을 사실처럼 말하지 말 것
- 모르는 것은 "확인이 안 됩니다" 또는 "로그를 직접 확인해 주세요"라고 솔직하게 답할 것
...
AI 선정 이유: {ai_reason if ai_reason else "퀀트 스코어 기반"}
