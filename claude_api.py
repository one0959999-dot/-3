...
- 답변은 간결하고 실용적이어야 합니다."""
    ...
    # BEST  : 환경변수로만 Opus 활성화 가능
    # 모델 등급 — 역할별로 구분해서 비용 최적화
    # FAST  : 매매 승인·위성 검토 등 반복 판단 (Haiku — 빠르고 저렴)
    # SMART : 채팅·시장분석 등 사용자 대화 (Sonnet)
    # DEFAULT: 기본값 (환경변수로 재정의 가능)
...
[
  ...
  ...
    ...
    def ai_select_core_stocks(self, candidates: list, n: int) -> list:
        ...
        prompt = f"""[코어 종목 최종 선정 요청]

━━ 코어 슬롯 투자 목표 ━━
• 보유 기간: 장기 (1년~수년, 지속 누적 매수)
• 목표: 주식 수를 꾸준히 늘리며 복리로 자산을 키우는 "기둥 종목"
• 우선순위: ① 한국 시장 섹터 대장주 (시가총액 상위, 독점적 시장지위)
             ② 실적 성장 지속 + 재무 안정 (부채비율 낮음, ROE 두 자릿수)
             ③ 글로벌 경쟁력 — 수출 비중↑, 환율 수혜 구조
             ④ 하락장에서도 버틸 수 있는 펀더멘털
• 배제: 단기 테마주, 적자 기업, 변동성이 지나치게 큰 소형주

━━ 퀀트 검증된 후보 (120일 정배열 통과) ━━
{candidate_text}

위 후보 중 "10년 뒤에도 한국 시장에서 살아남아 성장할 가능성이 높고,
매달 추가 매수해도 아깝지 않은" 종목 {n}개를 선정하세요.
단기 모멘텀보다 장기 펀더멘털과 산업 지위를 기준으로 판단하세요.

반드시 아래 JSON 형식으로만 답변하세요. 다른 텍스트는 절대 포함하지 마세요.
[
  {{"ticker": "종목코드", "reason": "선정이유(장기 보유 근거 포함)"}},
  ...
]"""
        try:
            resp = self.client.messages.create(
...
REASON: (핵심 근거 1~2줄)"""
    ...
    def chat(self, user_message: str, portfolio_context=None, stock_analysis_context=None) -> str:
        ...
        try:
            # 채팅은 Sonnet — 사용자 대화 품질 유지하면서 Opus 대비 80% 절감
            resp = self.client.messages.create(
