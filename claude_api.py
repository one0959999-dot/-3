...
- 답변은 간결하고 실용적이어야 합니다."""
    ...
    DEFAULT_MODEL  = os.environ.get("LASSI_CLAUDE_MODEL",       "claude-sonnet-4-5-20250929")
    _FAST_MODEL    = os.environ.get("LASSI_CLAUDE_FAST_MODEL",  "claude-sonnet-4-5-20250929")
    _SMART_MODEL   = os.environ.get("LASSI_CLAUDE_SMART_MODEL", "claude-sonnet-4-5-20250929")
...
──────────────────────────────────────────
【판단 지침】
• 위에 제공된 데이터만 근거로 사용할 것.
• 제공되지 않은 지표(KOSDAQ 상대강도, 외인수급 등)는 언급하지 말 것 — "Unverified" 표기 금지.
• N/A로 표시된 데이터도 근거로 쓰지 말 것.
• 자체 6개 체크리스트 등 임의 기준 생성 금지.
• 핵심 수치(RSI값, 볼린저밴드 위치, 거래량 배율, 5일선 위치 등) 구체적으로 인용할 것.
