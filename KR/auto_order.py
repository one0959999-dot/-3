...
def main(execute=False, max_buy=None, force=False, probe=False, budget_override=None, anytime=False):
    ...
    cap = max_buy if max_buy else deploy  # 캡 미지정 = 가용현금 전액
    print("[스캔] 전 종목 시세 수집...")
    ...
    cap_txt = f"{max_buy/1e4:,.0f}만" if max_buy else "현금전액"

    head = '🔴실행' if execute else '📋드라이런(주문0)'
    L = [f"🤖 자동주문 {head} — {meta['last_day']}"]
    L.append(f"[신규자금 배분] 가용현금 {deploy/1e4:,.0f}만 → 매수 {len(buys)}건 ≈{buy_val/1e4:,.0f}만 "
             f"(기존보유 불간섭, 캡 {cap_txt})")
