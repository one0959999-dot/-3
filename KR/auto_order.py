...
def _price_retry(toss, sym, tries=3, delay=0.6):
    """시세 조회 재시도 — 일시적 API 실패(오늘 KODEX200 시세실패 사례) 방어. 성공가(양수) or None."""
    for i in range(tries):
        try:
            p = toss.get_price(sym)
        except Exception:
            p = None
        if p and p > 0:
            return p
...
def plan_buyonly(target, toss, lowvol_cash, index_cash):
    ...
    ※저변동예산이 25종목 분산에 못 미치면(소액) 저변동 몫도 지수ETF로 몰아줌.
    반환 (buys, skipped, index_ok). index_ok=False = 지수 사려 했는데 시세실패로 못 삼(→ 상위서 그날 보류)."""
    ...
    index_want = index_cash > 0 and n_etf > 0   # 이번 회차 지수를 사야 하나
    index_done = False
    buys, skipped = [], []
    for t in target:
        is_etf = t['sleeve'] == '지수ETF'
        if not is_etf:  # 저변동
        ...
        sym = t['symbol']; lp = _price_retry(toss, sym)  # 재시도로 일시적 실패 방어
        if not lp or lp <= 0:
            skipped.append((t['name'], '시세실패(재시도후)')); continue
    ...
    index_ok = (not index_want) or index_done   # 살 필요 없었거나(예약분 대기) 실제로 샀으면 OK
    return buys, skipped, index_ok
...
def main(execute=False, max_buy=None, force=False, probe=False, budget_override=None, anytime=False):
    ...
    buys, skipped, index_ok = plan_buyonly(target, toss, lowvol_bud, index_bud)
    ...
    if not index_ok:
        L.append("  ⚠️ 지수 시세실패(재시도후) — 실행시 오늘 배분 전체 보류(저변동 편중 방지, 내일 재시도)")
    ...
    if eff < MIN_CASH_ORDER:
        ...
        return 0
    if not index_ok:
        # 지수 사려 했는데 시세실패(재시도후) — 저변동만 사면 편중. 그날 배분 전체 보류(현금 그대로, 내일 재시도).
        msg = ("⛔ 실행보류: 지수ETF 시세조회 실패(재시도후) — 저변동만 사면 비중이 편중되므로 "
               "오늘 배분 전체 보류(저변동 포함). 현금·보유 그대로, 내일 크론 재시도.")
    ...
    try:
        ...
        for s, q, nm, tlp in buys:
            ...
            lp = _price_retry(toss, s)  # 재시도로 일시적 시세실패 방어
