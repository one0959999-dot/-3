"""
봇 8단계 불장 검증 — 강불장에서 봇이 상승을 놓쳐 손해 보는지(봇의 비용) 확인.
봇헤지 = KOSPI 8단계 ~bear (베어면 현금). 강불장 2020-04~2021-06 (코로나 V회복).
종목: 에스앤에스텍(101490) + 삼천당제약(000250).
봇이 보유랑 비슷하면 = 이상적(불장 안 손해). 많이 지면 = 봇의 비용 발견.
실행: python3 KR/bull_robustness_backtest.py
"""
import pandas as pd
import numpy as np
...
def main():
    bear = kospi_is_bear()
    print("봇 8단계 불장검증 / 강불장 20.04-21.06 (수익% / MDD%)")
    for name, code in TICKERS.items():
        ...
        diff = bot[0] - bh[0]
        print(" → " + ("봇 승" if diff > 0 else "보유 승") + " (차이 {:+.0f}%p)".format(diff))
