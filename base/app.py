# -*- coding: utf-8 -*-
"""Lassi 대시보드 v6 — 포트폴리오 앱 스타일(파이차트, 촘촘). 조회 전용(구봇 0).

계좌=toss_api 직접조회. 원금=원가기준 자동계산=미실현 수익률. 자동로그인.
색: 한국식(빨강=상승/이익, 파랑=하락/손실). 파이=conic-gradient(외부 의존 0).
"""
...
app = Flask(__name__)
app.secret_key = os.environ.get('LASSI_SECRET', 'lassi-dash-v6')
...
def kr_snapshot(row):
    ...
    try:
        ...
        # 파이(conic-gradient): 지수ETF / 저변동 / 현금
        tot = out['total'] or 1
        cum = 0.0; stops = []
...
.dc{position:relative;width:108px;height:108px;flex-shrink:0}
...
.hicon{width:34px;height:34px;border-radius:11px;background:var(--soft);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:12px;color:var(--sub);flex-shrink:0}
...
<div class=note>{{now}} · 크론 자동매매 · 조회 전용 · <a href="{{url_for('dashboard')}}">↻ 새로고침</a></div>
...
<div class=card>
  <div class=lab style=margin-bottom:12px>포트폴리오 구성</div>
  <div class=donut>
    <div class=dc><div class=pie style="background:conic-gradient({{kr.conic}})"></div>
  ...
  {% if kr.alloc[0].pct < 40 %}<div class=cap>⚠️ 지수 비중 부족 · 현금 {{ '%.0f'|format(kr.alloc[2].pct) }}% 재배분 대기 (목표 지수50/저변동50)</div>{% endif %}
</div>
...
<div class=card>{% for k,v in status.items() %}<div class=st><span class=kk>{{k}}</span><span class=vv>{{v}}</span></div>{% endfor %}</div>

<div class=h>📜 최근 거래</div>
<div class=card>{% for t in trades %}<div class=st><span><span class="tag {{'s' if t.action=='SELL' else 'b'}}">{{ '매도' if t.action=='SELL' else '매수' }}</span>{{t.stock_name}}</span>
...
{% if not trades %}<div class="mut" style=text-align:center;padding:14px>거래 없음</div>{% endif %}</div>
